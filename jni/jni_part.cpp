...
void logFile(std::string message)
{
	...
	{
		ostringstream out;
		out<<"/sdcard/ObstAvoidanceLog_"<<currentDateTime()<<".txt";
		FILENAME = out.str();
	}
	std::ofstream ofs;
	ofs.open (FILENAME.c_str(), std::ofstream::out | std::ofstream::app);
	if(ofs.fail())
		__android_log_print(ANDROID_LOG_INFO, "VOO", "Failed to open %s", FILENAME.c_str());
	else{
		ofs <<currentDateTime()<<": "<< message <<std::endl;
		ofs.flush();
		ofs.close();
	}
}
...
int obstacleAvoidance()
{
	...
	str << "OO: L:"<<obstaclesL<<", M:"<<obstaclesM<<", R:"<< obstaclesR;
	logFile(str.str());
...
JNIEXPORT jintArray JNICALL Java_edu_cmu_ri_airboat_server_AirboatObstAvoidanceView_FindFeatures(
    ...
    if(frames.size()==N)
    {
		...
		for(int i=1; i<N; i++)
		{
			...
			std::ostringstream str;
			str <<"Computing optical flow - prevPts size = "<<prevPts.size();
			logFile(str.str());
			if(prevPts.size()>0)
